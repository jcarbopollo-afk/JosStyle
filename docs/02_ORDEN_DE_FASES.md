...
| Track | Nombre | Origen | Numeración | Estado |
|---|---|---|---|---|
| **FIT** | Entrega 4 — Fitness | Documento propio de Josué, 33 251 líneas | Fases 1–45 | 🟡 **EN MARCHA** — **36 de 45 hechas** (v3.83.0 → v3.118.0). 🏁 El sistema de rangos entero está cerrado, F15 a F25, 🏁 **el bloque de progreso físico también: F26, F27 y F28**, y 🏁 **el de Inteligencia: F29 a F35**, con la **F35** validando el catálogo —36 reglas, errores que paran el build y avisos que no— y archivando ejercicios sin perder su historial. La **F36**, primera del bloque de **Acabado**, cableó las puertas entre pantallas que faltaban —historial → ejercicio, foto → entrenamiento, ficha → sesión en curso—, guardó el peso corporal de cada sesión y puso un límite de error por área. La **F26** puso las fotos —son `saludFotos` desde la Fase 3, con su bucket ya creado: no hizo falta SQL de Josué—, la **F27** el comparador **ampliando la comparación de la F26 en vez de escribir una segunda**, y la **F28** convirtió Fitness → Progreso en el centro de seguimiento: seis sistemas en una pantalla, cada bloque leyendo de **un solo motor** y **sin una cifra que los mezcle** (su apartado 10). La **F29** abre el bloque de **Inteligencia** (F29–F35): el detalle de un ejercicio con su cabecera, su rango, su objetivo, sus variantes, el selector de métrica y seis periodos — **sin una sola lógica de progreso nueva** (todo sale de la F11 y llega por la F12) y **sin guardar nada**. De sus catorce componentes, **cinco ya estaban escritos**. La **F30** le da a los objetivos —que ya eran de la F14— su ciclo entero: distancia, historial, gráfica con su **línea de objetivo**, un cuarto tipo (**habilidad**) sin porcentaje ni gráfico, aviso de duplicado y el objetivo visible durante el entrenamiento — **sin predecir nada** (su apartado 34, tres veces) y **sin tocar el rango** (apartado 37). La **F31** dice **cuándo entrenó, cuánto, cómo se reparte y cómo va el plan** —último entrenamiento, semana en curso, calendario, constancia en una frase y «3 / 5 sesiones planificadas»— **sin un porcentaje, sin inventar descansos** (un día sin sesión es «sin registro») y **sin guardar nada**; y destapó cuatro fallos de antes, arreglados donde nacían: «7 días» que eran ocho en toda Fitness (**C-38**), un segundo catálogo de periodos, la «última sesión» de la F28 que era la más antigua y una sesión sin fecha que se mudaba a hoy. La **F32** pone en Tu Plan **qué toca hoy, qué toca después, qué hizo y qué está planificado**: la semana del plan se recorre, cada día dice lo que el plan tenía y lo que hizo, **ningún día afirma «Descanso»** (su apartado 5) y **cambiar de plan no reescribe el pasado** (**C-39**, `planesAnteriores`); y destapó que los días de un plan de la biblioteca **nacían con un id aleatorio en cada carga** (F5). La **F33** convierte «Reemplazar» en una sustitución **con niveles de compatibilidad** —muy similar, similar, alternativa y, solo si se pide, poco recomendable—, cada una con su porqué, filtros por material y entorno y la búsqueda a mano; **un solo motor** para el entrenamiento en vivo, el constructor y la F9, y **nada se transfiere** al sustituir (historial, progreso, objetivo, rango ni clasificación). Le faltaba un dato al catálogo —el **patrón de movimiento**— y va en la línea de cada ejercicio. La **F34** convierte *Fitness → Ejercicios* en una **biblioteca** —recientes del historial, favoritos, explorar por grupo, búsqueda por material, músculo y patrón con y sin acentos, filtros combinados con «Limpiar filtros»— y su ficha en una **ficha útil**: técnica en pasos, la progresión de una skill que se recorre tocándola, variantes, **las alternativas de la F33 con su nivel**, «Añadir a entrenamiento» y «Tu progreso» de la F29; **la biblioteca y la ficha ya existían (F2) y se amplían**, la ficha **no calcula nada** y **los favoritos de ejercicios nacen aquí**, como ids y con su normalizador. **La siguiente es la F35**, y el estado fase a fase está en `docs/12_ENTREGA4_FITNESS_ORDEN.md` |
...
| v3.1.0 | **SO F5** | Producción, integración y test final. **Y por fin la pantalla**: `Ajustes → Sonido y respuesta` con los siete controles del apartado 24, **cableada en `App.jsx`**. ⏸ Y lo primero que decía era **"todavía no suena nada"**, porque no había archivos — 🏁 **la SO F2 los trajo y ahora suena**: el aviso no estaba escrito a mano, `hoySuena` **cuenta los archivos que hay**. 🚨 **El motor no se reescribe**: la estructura de carpetas del apartado 2 **no se adopta**, con su motivo — y lo que el apartado quiere de verdad sí, con una comprobación de que **ninguna pantalla hace `new Audio(...)`**. ⚠️ El volumen multiplica **maestro × categoría × evento**: un clic suena por debajo de un récord. ⚠️ **Un perfil se deduce, no se guarda**: tocas una casilla y pasas a "Personalizado". 🚨 **Silencio no es "no pasa nada"**: el evento se sigue procesando, y la vibración es otro interruptor. ⚠️ Y cinco pruebas necesitan un teléfono |

| v3.1.1 | **🐛 Calendario** | **Las tres recurrencias estaban rotas desde la Fase 3.** 🚨 Un evento **diario no avanzaba nunca** (500 copias del mismo día, y no salía en ningún otro día del mes); uno **semanal avanzaba 6 días**, no 7; y uno **mensual del 31** saltaba al 2 de marzo y se quedaba pegado al día 3. Las dos primeras eran **el UTC de siempre** —`toISOString` sobre medianoche local, **quinta vez** en el proyecto— y ya usan `fechaLocalISO`; la tercera era encadenar `setMonth(+1)`, y ahora **mensual y anual se cuentan desde el ancla**. ⚠️ Había sobrevivido porque **no existía ninguna prueba del calendario**: ahora hay 38 comprobaciones, con los tres fallos clavados |

🏁 **LA ENTREGA 2 ESTÁ CERRADA: 110 de 110.** Bloques ME (4/4), BI (4/4), AR (4/4), FO (12/12),
RA (4/4), HT (12/12), 🏁 EH (65/65) y 🏁 **SO (5/5)**.
✅ La última, **SO F2 —la biblioteca de sonidos—, la cerró la otra conversación**: los 46 archivos
están en `public/sonidos/` y en `main`, y Josué lo confirmó el 2026-09-12. **No queda ninguna fase
de la Entrega 2 por construir.**
