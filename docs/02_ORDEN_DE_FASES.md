...
| Track | Nombre | Origen | Numeración | Estado |
|---|---|---|---|---|
| **FIT** | Entrega 4 — Fitness | Documento propio de Josué, 33 251 líneas | Fases 1–45 | 🟡 **EN MARCHA** — **28 de 45 hechas** (v3.83.0 → v3.110.0). 🏁 El sistema de rangos entero está cerrado, F15 a F25, y 🏁 **el bloque de progreso físico también: F26, F27 y F28**. La **F26** puso las fotos —son `saludFotos` desde la Fase 3, con su bucket ya creado: no hizo falta SQL de Josué—, la **F27** el comparador **ampliando la comparación de la F26 en vez de escribir una segunda**, y la **F28** ha convertido Fitness → Progreso en el centro de seguimiento: seis sistemas en una pantalla, cada bloque leyendo de **un solo motor** y **sin una cifra que los mezcle** (su apartado 10). **La siguiente es la F29 (análisis avanzado de rendimiento por ejercicio)**, y el estado fase a fase está en `docs/12_ENTREGA4_FITNESS_ORDEN.md` |
