...
| v2.8.0 | **EH F19** | Cuerpo e higiene: rutinas y recomendaciones. 🚨 **UN SOLO ALMACÉN PARA LOS DOS MÓDULOS, y la plantilla del enunciado es la prueba**: la *"Rutina diaria básica"* —Ducha, Higiene, Desodorante, Hidratación corporal— cruza `higiene` y `cuerpo`, así que con dos listas no cabría en ninguna. ⚠️ **Ni un motor nuevo**: cuarta llamada al de rutinas, quinta al de reglas y tercera al de productos; las seis frecuencias del apartado 6 van sobre las **cuatro** que el motor ya sabe. ⚠️ **Omitir es una TERCERA cosa**: uno hecho y tres omitidos es una rutina HECHA, y ni un contador de racha (D2-02). ⚠️ **Antes de recomendar comprar, lo que YA TIENE** (apartado 11, con sus palabras). ⚠️ **La plantilla y el pack se OFRECEN**: vigésimo y vigesimoprimer `aplicarPlan`, y en Chromium se comprueba que **verla no escribe nada**. ⚠️ **Los favoritos globales del apartado 18 NO existen** y se dice. 🐛 **Cinco fallos de la misma familia —la FORMA de lo que devuelve una función—**: `silenciadaEn` da un objeto y `!objeto` se llevaba todas las recomendaciones; `guardadas` son objetos, no ids; `compararGenerico` recibe un objeto, no filas; el paso se llama `etiqueta`, no `nombre`; y los `motivos` del normalizador son objetos. 🚨 **Y una regla invariante nueva**: dos `const` con el mismo nombre en el recorrido de Chromium no compilan, y costaba doce minutos verlo |
...
Quedan **26** fases de la Entrega 2: SO (2, con **F2 bloqueada**) y EH (24, ninguna bloqueada).
