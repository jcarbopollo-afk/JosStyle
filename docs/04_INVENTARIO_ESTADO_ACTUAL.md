...
| `fondos.js` | `TIPOS_FONDO`, `FONDOS_INCLUIDOS`, `POSICIONES_FONDO`, `DEFAULT_FONDO`, `normalizarFondo`, `resolverFondo`, `estilosDeFondo`, `estilosDeVelo`, `seleccionarFondo`, `ajustarFondo`, `restablecerFondo`, `tieneFondoGuardado`, `describirFondo` | ✅ **CREADO** (v1.36.0, FO F1). Sistema central de fondos. Se guarda en `apariencia.fondo`, **no en una clave nueva**. `resolverFondo` nunca devuelve null: el peor caso es el fondo normal de JosStyle (apartado 6). 101 comprobaciones |
