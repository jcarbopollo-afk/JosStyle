...
| `fondos.js` | `TIPOS_FONDO`, `FONDOS_INCLUIDOS`, `POSICIONES_FONDO`, `DEFAULT_FONDO`, `normalizarFondo`, `resolverFondo`, `estilosDeFondo`, `estilosDeVelo`, `seleccionarFondo`, `ajustarFondo`, `restablecerFondo`, `tieneFondoGuardado`, `describirFondo`, y desde FO F2: `orientacionDeFoto`, `datosDeFoto`, `encuadreInicial`, `validarFotoFondo`, `aplicarFoto`, `quitarFoto`, `tieneFoto` | ✅ **CREADO** (v1.36.0, FO F1). Sistema central de fondos. Se guarda en `apariencia.fondo`, **no en una clave nueva**. `resolverFondo` nunca devuelve null: el peor caso es el fondo normal de JosStyle (apartado 6). Ampliado en v1.37.0 (FO F2) con la fotografía. 152 comprobaciones |
