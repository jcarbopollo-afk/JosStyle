...
Estudios— y arregló lo que Josué encontró **usando la aplicación en su iPhone**. Casi nada se
...
| Saber qué se hizo en la **Entrega 3** (46 fases — 🏁 **cerrada**) | **`docs/11_ENTREGA3_ORDEN.md`** |
...
Vite, **20 685 pruebas unitarias** con Node repartidas en **193 suites** (5 de ellas de auditoría),
**3524 casos de renderizado real** con `react-dom/server`, **11 reglas invariantes** y **2538
comprobaciones sobre la aplicación de verdad en Chromium** — **26 758 comprobaciones**.

⚠️ **Estas cifras están contadas de la pasada verde de la v3.115.0, una a una**, con la suma del
propio log (`═══ TODO CORRECTO ═══`, 0 fallos). El salto desde la v3.114.0 es exactamente lo que
añadió la F33: +156 de Node en su suite, +60 de renderizado y +63 del recorrido. Y se vuelven a contar cada vez por lo que pasó en su día: las que hubo aquí —*"17 207 en
...
🏁 **LA ENTREGA 3 ESTÁ CERRADA: 46 de 46 (v3.67.0).** Hechas la **F1 (Pulido global, v3.10.0)**, la
...
NU—, y la **F41 (Estudios, home tipo teléfono y nueva arquitectura,
...
**F44 (exámenes, entregas y fechas, v3.65.0)**, la **F45 (apps de aprendizaje independientes,
v3.66.0)** y la **F46 (próximos eventos, resumen e integración final, v3.67.0)**, que 🏁 **CERRÓ
ESTUDIOS (6/6) Y LA ENTREGA 3 ENTERA (46/46)**. Con eso quedan **los siete bloques cerrados** —Hoy y
Calendario (10/10), Biblioteca (8/8), Productividad (7/7), Bienestar (1/1), Sueño (2/2), Nutrición
...
- 🐛 **UNA COMPROBACIÓN QUE MIRA LA PÁGINA ENTERA SE ROMPE CUANDO OTRA FASE AÑADE UN BLOQUE
  LEGÍTIMO** (FIT F31 → F28): la de la F28 prohibía «esta semana» en toda la pantalla de Progreso,
  y la F31 puso ahí su bloque de actividad, titulado —con todo el derecho— «Esta semana». Lo que la
  F28 prohíbe es **una cifra suya dicha como semana** («3 esta semana»), y eso es lo que mide ahora,
  con su comprobación de que la sigue cazando. **Una comprobación mide lo que protege, no la pantalla
  entera** (es `[role="dialog"]` en vez del documento, de la F27, otra vez).
...
- 🚨 **LA EXPLICACIÓN DE UN RANGO NO PROMETE CIFRAS** (FIT F20, `src/lib/explicacionRangos.js`):
  ...
  componente** (`RankExplanation`) para el global, el muscular y el de un ejercicio.

- 🚨 **QUIÉN MANDA EN UN RANGO SE DECIDE EN UN SOLO SITIO** (FIT F19, `src/lib/motorRangos.js`):
  sin sesiones, el cuestionario; con una o dos, la mezcla; con tres, solo el entrenamiento. Los
  umbrales viven en `UMBRALES_FUENTE` y hay una prueba que busca `if sesiones > 3` sueltos.
- 🚨 **Una mala sesión no baja el rango**: la puntuación es la mejor de las últimas cinco.

- 🚨 **LA LISTA DE EJERCICIOS DE UN MÚSCULO SALE DEL CATÁLOGO** (FIT F18, `src/lib/
  detalleMuscular.js`), nunca de una lista escrita a mano, y enseña también los que NO ha
  entrenado: es lo que hace real el «4 de 18 con datos».
- ⚠️ **Dos pantallas por músculo, a propósito**: la de Rangos contesta «qué nivel tengo y de dónde
  sale»; la de Progreso (F13), «cómo evoluciona mi rendimiento». Comparten el cálculo, no el sitio.
...
- 🚨 **LA PANTALLA DE RANGOS NO CALCULA** (FIT F16). `src/lib/pantallaRangos.js` pide
  ...
  vista buscando umbrales, fórmulas o `score`.
- ⚠️ **Rangos no duplica el detalle muscular**: manda un foco y lo abre Progreso (el de la F13).
...
- 🚨 **`fitness.objetivos` guarda SOLO el objetivo** (FIT F14): conseguido, porcentaje y valor actual
  ...
  celebre objetivos en el entrenamiento en vivo usa `objetivosQueConsigueLaSesion`.

- 🚨 **EL PROGRESO MUSCULAR REPARTE Y VOTA, NO COMPARA** (FIT F13, `progresoMuscular.js`): peso =
  porcentaje del catálogo; votan solo los comparables de la F11; sin datos nunca es descenso; uno solo
  lleva «poca información»; varios, mayoría ponderada. **Los Rangos (F15+) no deben reutilizar este
  voto como si fuera un nivel**: mide tendencia de rendimiento, no fuerza.

- 🚨 **FITNESS → PROGRESO ES `ProgresoView`** (FIT F12), con Resumen, Ejercicios y Fotos. El progreso
  por grupo muscular (F13) y las metas (F14) **van ahí**, sin otra navegación. Y su librería,
  `progresoEjercicios.js`, **no compara nada**: todo sale de la F11.
- 🚨 **Una pantalla que enseña series de una sesión las numera como el historial** (omitidas fuera):
  la F11 guarda `numero` en cada serie. Numerar por posición contradice al historial.
...
- 🚨 **UNA COLECCIÓN GUARDA REFERENCIAS, NO CONTENIDO** (E3 F21). Añadir una nota a «Estudios» guarda
  ...
  su contenido"* **no hay que programarlos**: salen de no tener copia. Ni un título, ni un texto, ni
  una URL copiados, con una prueba que lo barre.
- 🚨 **LA RELACIÓN VIVE DENTRO DE LA COLECCIÓN, Y ESO LO AUTORIZA EL ENUNCIADO** (E3 F21). Describe
  una tabla `COLLECTION_ITEMS` y a continuación dice *"si la base de datos actual usa relaciones
  específicas por tabla, **respetar la arquitectura existente**"*. La de JosStyle es `app_data`, una
  fila por (usuario, clave): no hay dónde poner una tabla de relación. Con `elementos: [{tipo, id}]`
  dentro salen gratis el borrado en cascada, el «un elemento en varias colecciones» y el aislamiento.
- 🚨 **CUANDO DOS FASES SE CONTRADICEN, MANDA LA QUE CONSTRUYE LA FUNCIÓN** (E3 F21). La BL F4 preparó
  un `coleccionId` por guardado; la BL F7 dice *"no asumir un `collection_id` único"*.
  `absorberColeccionId` lo convierte en una relación y lo deja a `null` **desde el normalizador**:
  dos fuentes de verdad para lo mismo es como se acaba enseñando una cosa distinta en cada pantalla.
- ⚠️ **UNA RELACIÓN HUÉRFANA NO SE LIMPIA AL BORRAR, Y ES A PROPÓSITO** (E3 F21): lo borrado va a
  *Eliminados recientes* y **vuelve**, así que quitar la relación en ese momento le arrebataría en
  silencio algo que él había organizado. No resuelve —no se cuenta ni se pinta— y `limpiarRelaciones`
  está para cuando el elemento ya no existe en ninguna parte.
- ⚠️ **UN ACENTO SE GUARDA COMO TOKEN, NUNCA COMO HEX** (E3 F21, y es la regla 2): `'info'`, no
  `'#6C99C9'`. Un hex guardado se queda fijo cuando Josué cambia de tema. Mismo reparto que
  `TIPOS_EVENTO_CALENDARIO.colorToken`.
...
- 🚨 **UN NÚMERO DEL ENUNCIADO PUEDE SER UNA MAQUETA, NO UN DATO** (E3 F33). El apartado 2 de
  ...
  objetivos son la Fase 3, así que `objetivo` es **un campo del indicador**: sin él se ve lo
  consumido y ya; con él aparecen el «de X» y la barra **sin tocar la pantalla**. Antes de pintar un
  número de un enunciado, mirar si existe el dato que representa.
- 🚨 **Y LA REGLA 7 PESA MÁS QUE UN APARTADO** (E3 F33): JosStyle no le pone objetivos calóricos
  estrictos a un chaval de 16 años. Ni siquiera se usa el **TDEE que Ajustes ya calcula**, porque
  sería la aplicación poniéndole una cifra. Los pondrá él.
- 🚨 **UN SELECTOR QUE NO CAMBIA NADA ES UN CONTROL DECORATIVO** (E3 F33, regla 8). El enunciado
  permitía que la zona de comidas fuera *"visual/no funcional si es necesario"* — pero el registro de
  comidas **ya funcionaba** desde la Fase 4, así que un botón muerto habría sido un paso atrás. Cada
  «+ Añadir» abre el formulario de siempre, y guarda **en el día y el momento elegidos**: el
  formulario escribía `todayISO()` a pelo, lo que habría dejado el selector decorativo.
- 🚨 **A LO GUARDADO ANTES NO SE LE INVENTA EL CAMPO NUEVO** (E3 F33): una comida sin `momento` se
  queda en `null` y cae en **Extras**, el cajón que el propio enunciado define. Escribirle
  `'desayuno'` sería inventarse cuándo se la comió.
- 🐛 **DOS VISTAS SIN NI UN CASO DE RENDERIZADO EN LA MISMA ENTREGA** (E3 F30 y F33): `HealthView` y
  `NutritionView`. **Al tocar una pantalla, comprobar primero que está en `smoke-vistas.jsx`** — que
  se pinte en producción no significa que la pruebe nadie.
- 🐛 **UN ESCENARIO DE PRUEBA CON LOS CAMPOS MAL NO FALLA: CALLA** (E3 F33, y EH F44 lo dijo primero).
  El de Nutrición escribía `kcal`, `prot` y `carbs`, y **nadie lee esos campos**: el resumen del hub
  venía calculando **0 kcal sobre una comida de 350** sin que ninguna prueba lo dijera.
- 🐛 **Y una comprobación mía volvió a buscar UNA CIFRA en vez del MECANISMO** (E3 F33): barría la
  librería buscando «2400» y saltaba con la línea que **demuestra** que el indicador sabe pintar un
  objetivo cuando se lo dan. Lo que se comprueba es que la pantalla pida el resumen **sin objetivos**.
...
- 🚨 **UN LÍMITE VA DESPUÉS DEL FILTRO, NUNCA ANTES** (E3 F46, y es un fallo real mío).
  `proximoDeAsignatura` cogía **los tres eventos más cercanos de todo Estudios** y luego se quedaba
  con los de esa asignatura: con dos asignaturas cargadas de exámenes, entrar en la tercera decía
  **«Nada próximo»** teniendo un examen el jueves. La pantalla se pintaba perfecta, el número salía
  y era otro. **Al reutilizar una función que topa y filtra, mirar en qué orden lo hace.**
- 🚨 **UNA FASE DE INTEGRACIÓN NO AÑADE UN SISTEMA: REPARTE EL QUE HAY** (E3 F46, apartado 21, que lo
  enumera: *"no añadir segundo calendario, segundo sistema de eventos, notificaciones
  independientes"*). Las **cinco** vistas nuevas leen todas de `fechasAcademicas()`, y por eso
  cambiar la fecha de un examen reordena el Home solo — **no hay nada que sincronizar porque no hay
  copia**. Es `hoy.js` (HT F6) y `agendaDia.js` (E3 F7) otra vez.
- 🚨 **UNA AFIRMACIÓN DE CONSISTENCIA SE EJECUTA, NO SE DESCRIBE** (E3 F46, apartado 19):
  `auditoriaConsistencia()` **crea** un examen, le cambia la fecha, lo borra y oculta un área, y mira
  que el Home cambie. Una tabla que dice *"todo sale de la misma fuente"* solo se cuenta a sí misma
  (EH F42).
- ⚠️ **LO DE HOY VA ARRIBA Y APARTE, Y ES LA MISMA LISTA** (E3 F46, apartado 4: *"no esconder un
  evento importante debajo de eventos futuros"*). No es un orden distinto: es un reparto en dos
  bloques — con dos listas habría que mantenerlas iguales a mano.
- ⚠️ **UN «VER TODOS» QUE LLEVA A LA LISTA QUE YA ESTÁS VIENDO ES UN BOTÓN QUE NO HACE NADA**
  (E3 F46, regla 8): solo aparece si queda algo fuera, incluidos los pasados.
- ⚠️ **UNA RUTA SE CALCULA, NO SE GUARDA** (E3 F46, apartado 7, y EH F37 lo dijo con las migas):
  *Estudios → Bachillerato → Biología → Examen* sale del evento en el momento, así que renombrar el
  área o mover la asignatura no la deja vieja.
- ⚠️ **LO PASADO NO SE BORRA NI SE ARCHIVA** (E3 F46, apartados 8 y 9): deja de ser próximo y sigue
  ahí, en su pestaña. Y lo que **no tiene fecha** no es ni próximo ni pasado: no se le inventa un
  sitio.
- ⚠️ **UNA AUDITORÍA DE CIERRE EJECUTA LAS CONDICIONES DE SUS FASES** (E3 F46, y la EH F64 y la
  E3 F40 lo dijeron antes): `FASES_ESTUDIOS` guarda las **funciones importadas**, así que renombrar
  una rompe la compilación y las doce casillas **se calculan**.
- ⚠️ **LO QUE NO SE CONSTRUYE SE DECLARA CON QUIÉN DECIDE** (E3 F46, apartado 23): las ramas sin
  sistema, el botón atrás del móvil y la sincronización entre dispositivos van escritos en
  `DEUDA_TECNICA`, no omitidos. Un informe que solo enumera lo verde miente por omisión.
- 🐛 **Y LA SEXTA EXCLUSIÓN A MANO DE LA MISMA REGLA** (E3 F46): `cierreEstudios` lo caza la
  expresión que busca `cierre` por el `cierre.js` de la EH F65, como ya pasó con `cierreNutricion`.
  Está avisado en la cabecera del archivo.

- 🚨 **UN MÓDULO QUE PIDE «OBJETIVOS» NO NECESITA UN SISTEMA DE OBJETIVOS** (E3 F45, apartado 8, y
  EH F28 lo dijo primero): JosStyle tiene los suyos desde la Fase 9, así que un área guarda **solo
  los ids** y la pantalla **dice dónde viven**. Y quien escribe los dos almacenes lo hace en **una
  sola llamada**: dos escrituras seguidas en el mismo turno se pisan (E3 F26).
- 🚨 **DOS ÁREAS DE LA MISMA PLANTILLA NECESITAN RAMAS CON IDS PROPIOS** (E3 F45): si
  `ramasDePlantilla` devolviera siempre los mismos ids, quitar una sección en Fútbol la quitaría en
  Ajedrez. Hay una comprobación de que dos llamadas dan los mismos nombres y **distintos ids**.
- ⚠️ **«PERSONALIZADA» NO ES UN VALOR GUARDADO** (E3 F45, apartado 2): es `null`, que ya significaba
  «sin tipo» desde la ES F2. Inventarle un id habría sido un valor para decir «ninguno» — y entonces
  hay dos formas de decir lo mismo.
- ⚠️ **UNA PLANTILLA SUGIERE, NO BLOQUEA** (E3 F45, apartado 5): después de aplicarla puede quitarlas
  todas. Y **«empezar desde cero» es una elección de verdad**: el área nace con `[]`, no con las de
  por defecto — `[]` es una decisión suya, no un hueco (la lección de la ES F2).
- ⚠️ **UNOS MINUTOS EN BLANCO SON `null`, NO 0** (E3 F45, apartado 10, y ya van cuatro veces con
  `Number(null)`): un cero diría que no practicó nada, y lo que pasa es que no lo apuntó. Y el
  resumen **suma solo los que los tienen**.
- 🐛 **AL BORRAR EN CASCADA, TODO LO QUE SE SACA DEL MÓDULO TIENE QUE IR A LA MISMA ENTRADA DE
  PAPELERA** (E3 F45, y es un fallo real que dejé yo en la E3 F43 y la E3 F44). Borrar un **área**
  quitaba de `estudios` sus temas, entregas y eventos **sin meterlos en la entrada**: restaurarla la
  habría devuelto sin ellos. Solo se vio al mirar la lista entera para añadir una cuarta colección.
  **Al añadir una lista que cuelga de algo, revisar las DOS cascadas**, no solo la suya.
- 🐛 **UN ÁREA DE APRENDIZAJE DECÍA «0 ASIGNATURAS»** (E3 F45, y lo cazó Chromium): el pie de la
  pantalla de área contaba asignaturas siempre, y Ajedrez no tiene. Es el cero de algo que en esa app
  **no existe** — el mismo error que el *«0 exámenes»* que la E3 F41 ya evitaba en las plaquitas.
- 🐛 **Y LA QUINTA EXCLUSIÓN A MANO DE LA MISMA REGLA** (E3 F45, tras `horarioEstructura`,
  `sonidoProduccion`, `rutinas` y `cierreNutricion`): la auditoría de EH busca `aprendizaje` por el
  `aprendizaje.js` de la EH F57, así que marcó `appsAprendizaje`. Está avisado en la cabecera del
  propio archivo, antes de que a nadie le cueste el rato.
...
- 🚨 **`Textarea` SE COMÍA EL `ref`** (E3 F20), y no lo veía nadie: el editor de Documentos lo usa
  ...
  del texto **sin que nada fallara**. Ya lo reenvía, como `TextInput`. Antes de pasarle un `ref` a un
  componente de `ui.jsx`, comprobar que lo acepta.
- 🚨 **UN DOCUMENTO SE PINTA CON ELEMENTOS DE REACT, NUNCA CON `dangerouslySetInnerHTML`** (E3 F20).
  Lo que hay dentro lo escribe Josué, pero un texto pegado de cualquier sitio no tiene por qué ser
  inofensivo, y ese camino no se vuelve a cerrar.
- ⚠️ **El formato de un documento es Markdown, a propósito** (E3 F20): un editor visual exigiría una
  librería nueva (DEP-26) y `contenteditable` en el iPhone es un campo de minas. Con texto plano, el
  autoguardado, la búsqueda por contenido, copiarlo entero y el índice salen gratis.
- 🚨 **UN AUTOGUARDADO CON RETARDO TIENE QUE GUARDAR AL CERRAR** (E3 F20): sin eso, escribir una
  frase y salir en menos de un segundo la pierde. Y su retardo es **más largo** que el del buscador.
- ⚠️ **Las etiquetas se guardan normalizadas y DENTRO de su elemento** (E3 F20): `#Claude`, `claude`
  y `#claude` son la misma, y una tabla aparte en un almacén de una fila por clave habría que
  sincronizarla a mano y dejaría etiquetas colgando de documentos borrados.
- ⚠️ **Una relación futura se DECLARA, no se deja como cinco campos vacíos** (E3 F20): un campo que
  nadie puede rellenar es media función (regla 8).
- 🐛 **Y para comprobar si un archivo TOCA otra lista hay que quitar también las CADENAS** (E3 F20):
  `donde: 'bibliotecaArchivos'` es una tabla que documenta, y *"apuntes largos"* es español
  corriente. Las dos hacían saltar el barrido con código que estaba bien.
...
- 🚨 **UNA FUNCIÓN QUE NADIE LLAMA NO FALLA NUNCA.** Van **dos** en esta entrega:
  ...
  probadas y **muertas**, y ninguna la veían el build, el renderizado ni las pruebas de Node. Antes
  de dar por hecho que algo funciona, **comprobar que alguien lo llame**.
- ⚠️ **Archivar y Eliminar son dos acciones distintas** (E3 F5), y el enunciado prohíbe sustituir una
  por otra. Un horario **no va a la papelera**, así que su aviso dice "permanente" y no promete
  recuperarlo. Solo eliminar pide confirmación.
- 🐛 **El limpiador de comentarios de una prueba NO puede usar un patrón de "llave, comentario,
  llave"** para los comentarios JSX (E3 F5): se come el código desde el primer `(() => {` con un
  comentario dentro — **500 líneas** en este caso. Quitar primero los bloques, después las llaves
  vacías.
...
- 🐛 **UN CAMPO QUE NO LEE NADIE NO FALLA NUNCA** (E3 F3). `CATEGORIAS_ARMARIO` declaraba un `icono`
  ...
  una camiseta durante meses** y ocho categorías compartían dibujo. Antes de añadir un campo a un
  catálogo, comprobar que alguien lo lea.
- ⚠️ **Añadir una categoría al armario es añadir su línea en `CATEGORIAS_ARMARIO` Y en
  `ICONOS_CATEGORIA`** (`src/components/iconosPrenda.jsx`): una es de datos y la otra de componentes
  de React. Los iconos propios van en la gramática de Lucide —24×24, solo trazo, `currentColor`,
  grosor 2, remates redondeados— y salen de UNA base `<svg>`. **Nada de emojis.**
...
- 🚨 **Faltan dos, y siguen faltando**: la **sincronización** entre dispositivos (el último en
  escribir gana) y el **móvil** (nadie ha abierto esto en un iPhone). Cualquier fase que las declare
  resueltas está mintiendo.

- 🚨 **`/api/ask-ai` no pide autenticación y CUALQUIERA puede gastar dinero con él** (EH F63). Los
  límites de tamaño ya están; **la autenticación no**, porque ese endpoint lo usan seis módulos más y
  la decisión es de Josué. Está escrito en `HALLAZGO_ENDPOINT` con el arreglo exacto.
- ⚠️ **El aislamiento entre usuarios es de RLS, nunca de la pantalla** (EH F43 y F63): esconder un
  botón no protege nada. Cualquier tabla nueva necesita sus cuatro políticas `auth.uid() = user_id`.

- 🚨 **Nada de alturas fijas por debajo de 44 px donde haya texto** (EH F62): con la letra del
  sistema subida, corta la frase. `alturasQueCortanTexto()` lo lee de la vista.
- 🚨 **Ni una palabra técnica en un texto que vea el usuario** (EH F62): `null`, `token`, `JSON`…
  `palabrasTecnicasEnPantalla()` mira lo visible, no el código. Y un error dice **qué corregir**,
  nunca "Error" a secas.
...
- ⚠️ **El resumen NO depende del aviso** (EH F59, apartado 8): `generarResumen()` no mira `avisar`.
  Al revés sí: desactivar el resumen apaga el aviso.
- ⚠️ **El historial de resúmenes guarda fechas y números, nunca el texto** (EH F59): el texto se
  vuelve a componer. Guardarlo dejaría resúmenes viejos escritos de otra forma tras cada retoque.

- 🚨 **Un insight con pocos datos no se enseña** (EH F58): cada tipo declara su `minimo` y por debajo
  se enseña la frase de "todavía no hay bastante". No rebajes un mínimo para que salga algo.
- 🚨 **Ni un insight puede sonar a reproche** (EH F58): `PALABRAS_DE_PRESION` se busca en los textos
  generados. Un cambio se cuenta con "más" y "menos", nunca con "mejor" y "peor".
- ⚠️ **Los números de los insights salen de `METRICAS_PROGRESO`** (F35): esta fase solo compara
  ventanas de tiempo. Una segunda lista de métricas acabaría contradiciendo la pantalla de Progreso.

- 🚨 **Sin el permiso de la F56 no se aprende nada** (EH F57): `aprender()` devuelve el estado tal
  cual. No apuntes señales "por si acaso": eso es lo que el apartado 16 prohíbe.
- 🚨 **Una preferencia explícita no la pisa ninguna cantidad de comportamiento** (EH F57): si él dijo
  una cosa y hace otra, se **pregunta** (`contradicciones()`), nunca se cambia sola.
- 🚨 **Borrar lo aprendido no toca sus datos** (EH F57): `borrarAprendizaje()` se lleva las
  deducciones y deja perfumes, rutinas y registros intactos. Borrar también lo explícito se pide aparte.
- 🐛 **En `test-app-real.mjs`, `pulsar` ESPERA a que el botón aparezca** (EH F57): no añadas
  `waitForTimeout` fijos antes de pulsar. Bajo carga producen **rojos falsos en cascada** — ya ha
  pasado dos veces.
...
- 🚨 **El backlog es `backlog()`, y se DERIVA** (EH F55): del `SE_POSPONE` de la F48, de
  `LO_QUE_FALTA` de la F54 y del fallo a medias de la F52. No escribas una segunda lista: se queda
  vieja el mismo día.
- ⚠️ **Antes de añadir una función, pásala por `evaluarFuncion()`** (EH F55): si no aporta, no se
  añade; si duplica, se **integra**; si complica la interfaz, se **replantea**.
...
- 🚨 **Antes de tocar Estilo de hombre: leer `docs/08_ESTILO_DE_HOMBRE_TECNICO.md`** (EH F53). Es el
  ...
  tocarse.
- 🚨 **Ese documento NO se edita a mano** (EH F53): se genera con
  `node --import ./scripts/resolver-vite.mjs scripts/generar-doc-eh.mjs` a partir de
  `src/lib/documentacionEH.js`. Si añades un módulo y no regeneras, la verificación se pone roja.
- ⚠️ **Una librería nueva de Estilo de hombre va a `LIBRERIAS_EH`** (EH F48 y F53): la que no está en
  esa lista **no la audita nadie** —ni privacidad ni duplicados— y su silencio parece un aprobado.
  Ya se coló cuatro veces.
